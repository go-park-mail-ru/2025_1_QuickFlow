...
type PostService interface {
    ...
    FetchCreatorPosts(ctx context.Context, creatorId uuid.UUID, numPosts int, timestamp time.Time) ([]models.Post, error)
...
func (f *FeedHandler) FetchUserPosts(w http.ResponseWriter, r *http.Request) {
    ...
    posts, err := f.postService.FetchCreatorPosts(ctx, user.Id, feedForm.Posts, ts)
    appErr := errors2.FromGRPCError(err)
    if appErr != nil && appErr.HTTPStatus != http.StatusNotFound {
...
func (f *FeedHandler) FetchCommunityPosts(w http.ResponseWriter, r *http.Request) {
    ...
    posts, err := f.postService.FetchCreatorPosts(ctx, community.ID, feedForm.Posts, ts)
    appErr := errors2.FromGRPCError(err)
    if appErr != nil && appErr.HTTPStatus != http.StatusNotFound {
