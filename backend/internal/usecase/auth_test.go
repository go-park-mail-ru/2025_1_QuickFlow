...
func TestCreateUser(t *testing.T) {
	...
	mockUserRepo.EXPECT().IsExists(ctx, user.Username).Return(false, nil).Times(1)
...
func TestCreateUser_AlreadyExists(t *testing.T) {
	...
	ctx := context.Background()
	user := models.User{Username: "existinguser", Password: "securepassword"}

	mockUserRepo.EXPECT().IsExists(ctx, user.Username).Return(true, nil).Times(1)
...
func TestGetUser(t *testing.T) {
	...
	user := models.User{Id: uuid.New(), Username: loginData.Login, Password: loginData.Password}
