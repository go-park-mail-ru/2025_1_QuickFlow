...
func (m *MessageHandler) GetMessagesForChat(w http.ResponseWriter, r *http.Request) {
    ...
    logger.Info(ctx, fmt.Sprintf("Fetching feed for user %s with %d posts with timestamp %v (autogenerated: %t)", user.Login, messageForm.MessagesCount, messageForm.Ts, !r.URL.Query().Has("ts")))
    ...
    case errors.Is(err, usecase.ErrNotParticipant):
        logger.Info(ctx, fmt.Sprintf("User %s is not a participant in chat %s", user.Login, chatId))
    ...
    case err != nil:
        ...
        http2.WriteJSONError(w, "Failed to load messages", http.StatusInternalServerError)
        return
    }

    logger.Info(ctx, fmt.Sprintf("Fetched %d messages for user %s", len(messages), user.Login))
...
func (m *MessageHandler) SendMessageToUsername(w http.ResponseWriter, r *http.Request) {
    ...
    logger.Info(ctx, fmt.Sprintf("User %s requested to send message", user.Login))
    ...
    userRecipient, err := m.authUseCase.GetUser(ctx, models.LoginData{Login: username})
