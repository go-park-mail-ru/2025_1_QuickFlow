...
type PostUseCase interface {
	FetchFeed(ctx context.Context, user models2.User, numPosts int, timestamp time.Time) ([]models2.Post, error)
	FetchRecommendations(ctx context.Context, user models2.User, numPosts int, timestamp time.Time) ([]models2.Post, error)
	FetchUserPosts(ctx context.Context, user models2.User, numPosts int, timestamp time.Time) ([]models2.Post, error)
	AddPost(ctx context.Context, post models2.Post) (models2.Post, error)
	DeletePost(ctx context.Context, user models2.User, postId uuid.UUID) error
	UpdatePost(ctx context.Context, update models2.PostUpdate, userId uuid.UUID) (models2.Post, error)
}
...
func (f *FeedHandler) GetFeed(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, usecase2.ErrInvalidNumPosts) {
		...
		return
	} else if errors.Is(err, usecase2.ErrInvalidTimestamp) {
...
func (f *FeedHandler) FetchUserPosts(w http.ResponseWriter, r *http.Request) {
	...
	requester, ok := ctx.Value("user").(models2.User)
	if !ok {
	...
	for i := range postsOut {
		postsOut[i].Creator = forms.PublicUserInfoToOut(publicUserInfo, models2.RelationSelf)
	}
